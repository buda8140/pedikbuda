...
/// Uses ONLY verified 9-byte command formats
struct LEDProtocol {

    // MARK: - Lotus Lantern X "App Mode" Priming Packets
    // These packets unlock interactive BLE mode on many ELK-BLEDDM controllers
    ...
    enum ProtocolVariant: Int, CaseIterable {
        ...
        case b = 1 // BLEDOM legacy (01 FF)
        case c = 2 // ELK specific (04 04)
        case d = 3 // Simple power variant
    }
    ...
    static func power(on: Bool, variant: ProtocolVariant) -> [UInt8] {
        ...
        case .b:
            // ON: 7E 00 04 01 FF 00 00 00 EF
            return on
                ? [0x7E, 0x00, 0x04, 0x01, 0xFF, 0x00, 0x00, 0x00, 0xEF]
                : [0x7E, 0x00, 0x04, 0x00, 0x00, 0x00, 0xFF, 0x00, 0xEF]

        case .c:
            // ON: 7E 04 04 01 00 00 FF 00 EF
            return on
                ? [0x7E, 0x04, 0x04, 0x01, 0x00, 0x00, 0xFF, 0x00, 0xEF]
                : [0x7E, 0x00, 0x04, 0x00, 0x00, 0x00, 0xFF, 0x00, 0xEF]

        case .d:
            // ON: 7E 00 01 FF 00 00 00 00 EF
            // OFF: 7E 00 01 00 00 00 00 00 EF
            return on
                ? [0x7E, 0x00, 0x01, 0xFF, 0x00, 0x00, 0x00, 0x00, 0xEF]
    ...
    static func color(
        ...
        // Exact 9-byte packet: 7E 00 05 03 R G B 00 EF
        if isGRB {
